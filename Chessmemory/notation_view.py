...
class NotationView(gtk.TextView):
    def __init__(self):
        ...
        self.textbuffer.create_tag("node", weight=pango.WEIGHT_BOLD, size_points=14)
        self.textbuffer.create_tag("comment", foreground="darkred", size_points=12)
    ...
    def update_selected_node(self):
        ...
        if start:
            ...
            self.textview.scroll_to_iter(start, 0, use_align=True, yalign=0)
    
    # Recursively insert the node tree
    def insert_nodes(self, node, level=0, ply=0, result=None, event=None):
        buf = self.textbuffer
        newLine = True
        ...
        if event:
            buf.insert_with_tags_by_name(end_iter(), "["+event+"]\n\n", "comment")
    ...
    def update(self):
        ...
        if self.board.game and self.board.game.notation.nodes[0]:
            self.insert_nodes(self.board.game.notation.nodes[0], result=self.board.game.keys["Result"], event=self.board.game.keys["Event"])
